# Plan de Implementación — GMT Link Módulos 1-4 (cierre de gaps)
...
| Gap (hoy roto/ausente) | Fase |
|---|---|
| RBAC dinámico inexistente; `Asset` sin `createdById`; logo chico | **F1** |
| Builder M1: jerarquía fija, sin meta-modelo, nomenclatura no por servicio | **F2** |
| UI matriz de roles / asignación (M4) | **F3** |
| Horas extra sin proyecto/autorizador/entrada-salida; vehículo sin gate de checklist; capture reembolso | **F4** |
| Bloques visuales de informe (M2) | **F5** |
| Reverse-queries con herencia (promoción a Opción C) | **F6 (opcional)** |

**Dependencias:** F1 desbloquea todo. F2 antes de F5 (template lee `FieldDef`). F3 tras F1. F4 usa `usersWithPermissionOnProject` de F1. F6 opcional, tras F1.
