...
import {
  VEHICLE_CHECKLIST_DEFAULT,
  ASSET_STATUS_LABELS,
  type UserOption,
  type AssetDetailTarget,
...
export function AssetDetailView({
  ...
  initialTarget = null,
  onBack,
}: AssetDetailViewProps): ReactNode {
  ...
  const handleUpdateSection = (
    ...
    setTplSections((prev) => prev.map((s) => (s.id === sectionId ? { ...s, [field]: value } : s)));
  };
  ...
  const handleSubmitChecklistAnswers = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const submission = await submitChecklistAnswers(id, {
      ...
      if (err instanceof ApiError && /firmar el checklist/i.test(err.message)) {
        await refreshUser();
        toast.info(
          'La firma del checklist pasó a ser obligatoria. Vuelve a enviarlo para firmarlo.',
        );
        return;
      }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        {asset.publicToken ? (
          <QrFichaPublica publicToken={asset.publicToken} code={asset.code} nombre={asset.name} />
        ) : null}
      </div>
              ...
              <Card>
                      ...
                      <>
                        <div className="flex justify-between border-b pb-2">
                          ...
                          <span className="font-medium">
                            {asset.inUseBy
                              ? `${asset.inUseBy.firstName} ${asset.inUseBy.lastName}`
                              : 'Desconocido'}
                          </span>
                        </div>
                        <div className="flex justify-between border-b pb-2">
                          ...
                          <span className="font-medium">
                            {asset.inUseSince
                              ? new Date(asset.inUseSince).toLocaleString('es-CL')
                              : 'N/A'}
                          </span>
                        </div>
                      </>
                      ...
                      <div className="col-span-full mt-2 bg-muted/20 border p-3 rounded-lg">
                        ...
                        <div className="grid grid-cols-1 gap-2">
                          <div className="flex justify-between text-xs">
                            ...
                            <span className="font-medium text-foreground">
                              {asset.manufacturer || 'No declarado'}
                            </span>
                          </div>
                          <div className="flex justify-between text-xs">
                          ...
                          {asset.type === 'VEHICULO' && (
                            <div className="flex justify-between text-xs">
                              ...
                              <span className="font-medium text-foreground">
                                {asset.vehicleSubtype
                                  ? VEHICLE_SUBTYPE_LABELS[asset.vehicleSubtype]
                                  : 'No declarado'}
                              </span>
                            </div>
                          )}
                              ...
                              <div className="flex justify-between text-xs">
                                ...
                                <span className="font-medium text-foreground">
                                  {meta.calibrationDate
                                    ? formatDate(meta.calibrationDate)
                                    : 'No declarada'}
                                </span>
                              </div>
                            </>
                          ...
                          {asset.type === 'VEHICULO' && (
                            <>
                              ...
                              <div className="flex justify-between text-xs">
                                ...
                                <span className="font-medium text-foreground">
                                  {meta.year ? String(meta.year) : 'No declarado'}
                                </span>
                              </div>
                            </>
                          )}
                ...
                <CardFooter className="bg-muted/10 border-t flex justify-end gap-4 items-center">
                  <div className="flex flex-wrap gap-2 justify-end">
                    ...
                    {asset.status === 'EN_USO' &&
                      !activeCycle &&
                      (asset.inUseById === profile?.id || isAdmin) && (
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => void handleReleaseUse()}
                          disabled={actioning !== null}
                        >
            ...
            <CardContent className="flex flex-col gap-6">
              ...
              {template && template.status === 'EN_REVISION' && (
                <div className="border border-amber-500/30 bg-amber-500/5 p-4 rounded-lg flex flex-col gap-3 text-xs">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-semibold text-amber-500">
                        Nueva versión de checklist en revisión
                      </span>
                    </div>
                  ...
                  <p className="text-muted-foreground">
                    La plantilla de checklist ha sido editada y se encuentra pendiente de aprobación
              ...
              {template && template.status === 'RECHAZADO' && (
                <div className="border border-rose-500/30 bg-rose-500/5 p-4 rounded-lg flex flex-col gap-2 text-xs">
                  ...
                  <p className="text-muted-foreground">
                    Esta plantilla fue rechazada por el revisor. Debe configurarse una nueva
                    versión.
                  </p>
              ...
              {showTplConfig && template && (
                <div className="border border-border p-4 rounded-lg bg-card/25 flex flex-col gap-4">
                  <p className="text-xs font-semibold text-foreground flex items-center gap-2">
                    <Settings className="size-3.5 text-primary" /> Diseñador de Plantilla de
                    Checklist
                  </p>
                  ...
                  <div className="flex flex-col gap-2 border border-border/60 rounded-md p-3 bg-card/30">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-[11px] font-semibold text-muted-foreground flex items-center gap-1.5">
                        <Layers className="size-3.5 text-primary" /> Secciones (páginas del
                        formulario)
                      </p>
                    ...
                    {tplSections.length === 0 ? (
                      <p className="text-[11px] text-muted-foreground">
                        Sin secciones: el formulario se llena en una sola página. Agrega secciones
                      ...
                      <div className="flex flex-col gap-2">
                        {tplSections.map((s, i) => (
                          <div
                            key={s.id}
                            className="flex flex-col gap-1.5 border border-border/50 rounded p-2 bg-background"
                          >
                            <div className="flex items-center gap-2">
                            ...
                            <Input
                              ...
                              value={s.description ?? ''}
                              onChange={(e) =>
                                handleUpdateSection(s.id, 'description', e.target.value)
                              }
                  ...
                  <div className="flex flex-col gap-3">
                    <p className="text-[11px] font-semibold text-muted-foreground">
                      Preguntas y Puntos de Control ({visibleTplItems.length})
                    </p>
                    ...
                    {visibleTplItems.length === 0 ? (
                      ...
                      <div className="space-y-3">
                        {visibleTplItems.map((item, index) => (
                          <div
                            key={item.id}
                            className="flex flex-col gap-3 p-3 border rounded bg-card/40 text-xs"
                          >
                            <div className="flex flex-col md:flex-row md:items-center gap-3">
                              ...
                              <div className="flex-1">
                                <Input
                                  value={item.label}
                                  onChange={(e) =>
                                    handleUpdateItemInTpl(item.id, 'label', e.target.value)
                                  }
                              ...
                              <div className="w-full md:w-36">
                                <Select
                                  ...
                                  value={item.type}
                                  onChange={(e) =>
                                    handleUpdateItemInTpl(item.id, 'type', e.target.value)
                                  }
                              ...
                              <div className="flex items-center gap-1.5">
                                <input
                                  ...
                                  onChange={(e) =>
                                    handleUpdateItemInTpl(item.id, 'required', e.target.checked)
                                  }
                                  className="size-3.5 rounded border-gray-300"
                                />
                                <Label htmlFor={`req-${item.id}`} className="text-xs select-none">
                                  Obligatorio
                                </Label>
                              </div>
                              ...
                              <div className="flex flex-col gap-2 border-t border-border/50 pt-2">
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        <button
                                          type="button"
                                          title={
                                            isFail ? 'Quitar marca de falla' : 'Marcar como falla'
                                          }
                                          onClick={() => {
                                        ...
                                        <button
                                          ...
                                          title="Quitar opción"
                                          onClick={() => {
                                            const opts = (item.config?.options ?? []).filter(
                                              (o) => o !== opt,
                                            );
                                            const fails = (item.config?.failOptions ?? []).filter(
                                              (f) => f !== opt,
                                            );
                                            handleUpdateItemConfig(item.id, {
                                              options: opts,
                                ...
                                <label className="flex items-center gap-1.5 select-none">
                                  <input
                                    ...
                                    onChange={(e) =>
                                      handleToggleRequireObs(item.id, e.target.checked)
                                    }
                                    className="size-3.5 rounded border-gray-300"
                                  />
              ...
              {!showTplConfig && template && template.status === 'APROBADO' && (
                <form
                  onSubmit={handleSubmitChecklistAnswers}
                  className="border border-dashed p-4 rounded-lg bg-card/25 flex flex-col gap-4"
                >
                  <div className="flex items-center gap-2">
                    <ClipboardCheck className="size-4 text-emerald-500" />
                    <p className="text-xs font-semibold text-foreground">
                      Ejecutar Inspección Diaria (Checklist Activo)
                    </p>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4">
          <Card className="w-full max-w-sm bg-card shadow-lg border border-border animate-in fade-in zoom-in duration-200">
            <CardHeader>
              ...
              <CardDescription>Reclamas el activo y abres el checklist inicial.</CardDescription>
            </CardHeader>
            <CardContent>
