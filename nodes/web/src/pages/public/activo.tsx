...
export default function PublicAssetPage(): ReactNode {
  ...
  // Id del documento que se está descargando, para su spinner. Uno a la vez
  // basta: el operador toca un documento, no diez.
  const [descargando, setDescargando] = useState<string | null>(null);

  /**
   * Pide la URL fresca del documento y la abre. Se abre en una pestaña nueva en
   * vez de navegar: si el navegador decide MOSTRAR el PDF en lugar de bajarlo,
   * el operador no pierde la ficha que estaba mirando.
   */
  async function descargarDocumento(docId: string): Promise<void> {
    if (!token) return;
    setDescargando(docId);
    try {
      const { url } = await getPublicAssetDocumentUrl(token, docId);
                        ...
                        <div className="flex min-w-0 flex-col">
                          ...
                          {doc.expiresAt && (
                            <span className="text-xs text-muted-foreground">
                              Vence el {formatearFecha(doc.expiresAt)}
                            </span>
                          )}
                        </div>
