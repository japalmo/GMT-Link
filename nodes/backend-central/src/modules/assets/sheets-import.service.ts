 ...
 * ── Lo que NO hace ─────────────────────────────────────────────────────────
 *
 * No crea plantillas de checklist ni las aprueba: eso tiene su propio flujo de
 * revisión y no puede ocurrir como efecto colateral de una importación. Un
 * vehículo sin plantilla se reporta y sus filas quedan fuera.
 *
 * No adivina vehículos ni personas. Una patente que no calza se reporta con su
 * conteo para que alguien decida; el conductor se guarda como texto informativo
 * y nunca como autoría.
 */
@Injectable()
export class SheetsImportService {
  ...
  async importar(): Promise<ResumenImportacion> {
    ...
    // 3. Resolver el vehículo de cada uno.
    ...
    this.logger.log(
      `Importación de checklists: ${resumen.importadas} nuevos, ${resumen.yaExistian} ya estaban, ` +
        `${resumen.descartadas.length} descartados, ${resumen.sinVehiculo.length} patentes sin vehículo ` +
